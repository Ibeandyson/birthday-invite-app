...
export default function CheckInForm() {
  ...
  const [guestInfo, setGuestInfo] = useState<{ name: string; isAlreadyCheckedIn: boolean } | null>(null)
  ...
  const onSubmit = async (data: CheckInFormData) => {
    setIsLoading(true)
    try {
      const result = await checkInGuest(data.uniqueCode)
      if (result.success) {
        setGuestInfo({
        ...
        toast.error(result.error || 'Check-in failed. Please verify your code.')
      }
    } catch (error) {
    ...
    <div>
      <div className="text-center mb-8">
        ...
        <p className="text-lg text-gray-600 fade-in">
          Enter your unique code to check in at the event
        </p>
      </div>

      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 slide-up">
        <div>
          <label htmlFor="uniqueCode" className="block text-sm font-semibold text-gold-800 mb-2">
            Unique Check-in Code *
          </label>
          <input
            {...register('uniqueCode', {
              required: 'Check-in code is required',
              pattern: {
                value: /^[A-Z0-9]{6}$/,
                message: 'Code must be 6 characters (letters and numbers only)'
              }
            })}
            type="text"
            id="uniqueCode"
          ...
          <p className="text-sm text-gray-500 mt-2">
            Enter the 6-character code you received via email
          </p>
        </div>
      ...
      <div className="mt-8 p-4 bg-cream-50 rounded-lg border border-cream-200">
        ...
        <p className="text-sm text-gray-600">
          If you don't have your check-in code, please contact the event organizer or check your email for the registration confirmation.
        </p>
      </div>
    </div>
  )
